 ...
 * @param args 格式化的参数。
 * @return 返回已处理的字符串。
 * @example formatLog("Hello, {bold:name}", {name: "xld"}) // "Hello, \x1b[1mxld\x1b[39m"
 * @example formatLog("Hello, {red:name}", {name: "xld"}) // "Hello, \x1b[31mxld\x1b[39m"
 * @example formatLog("{Error}: {bold:message}", {message: "foo..."}) // "\x1b[31mError\x1b[39m: \x1b[1mfoo...\x1b[39m"
 */
export function formatLog(value: string, args?: Object) {
    return value.replace(/\{(?:([a-z]+):)?([^}]+)\}/g, (all, color: string, key: string) => {
        if (args != null && key in args) {
            key = (args as any)[key];
        }
        if (color && typeof ConsoleColor[color as any] === "number") {
            return addLogColor(key, ConsoleColor[color as any] as any);
        }
        return key;
    });
}
