...
var __tpack__ = __tpack__ || {
    ...
    _fetch: function (baseUrl) {
        ...
        if (module) {
            ...
            // 生成模块键。
            var id = module.url.toLowerCase();

            // 保存到模块缓存。
            __tpack__.modules[id] = module;
    ...
    resolve: function (baseUrl, url) {
        ...
        link.href = notAbsolute ? baseUrl + "/../" + url : url;
    ...
    get: function (id) {
        ...
        if (module.exports && module.exports["__esModule"] && module.exports["default"]) {
            var obj = module.exports["default"];
            for (var key in obj) {
     ...
     * @returns {object} 返回模块的导出对象。 
     */
    require: function (url, callback, baseUrl, arg) {

        // 同时载入多个模块。
        if (typeof url !== "string") {
            for (var exportsList = [], count = url.length, i = 0; i < url.length; i++) {
                __tpack__.require(url[i], callback && function (exports, i) {
                    exportsList[i] = exports;
                    --count <= 0 && callback.apply(this, exportsList);
                }, baseUrl, i);
            }
            return;
        }
        ...
        __tpack__._loadings[id] = function () {
            ...
            callback && callback(__tpack__.get(id), arg);
        };

        // 模块未加载才继续。
        if (oldLoading) return;
